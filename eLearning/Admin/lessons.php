...
while($row=$result->fetch_assoc()){
    ...
    <tbody>';
        while($row=$result->fetch_assoc()){
            ...
            <button type="submit" class="btn btn-info mr-3" name="view" value="View"><i class="fas fa-pen">
            </i></button></form> <form action="" method="POST" class="d-inline">
            <input type="hidden" name="id" value='.$row["lesson_id"].'>
            <button type="submit" class="btn btn-secondary mr-3" name="delete" value="Delete">
            <i class="fa fa-trash-alt"></i> </button></form></td></tr>';
        }
        echo '</tbody>
        </table>';
    }
        ...
        if(isset($_REQUEST['delete'])){
            ...
            if($conn->query($sql)){
                echo '<meta http-eqiuv="refresh" content="0;URL=?deleted" />';
            }
            else{
